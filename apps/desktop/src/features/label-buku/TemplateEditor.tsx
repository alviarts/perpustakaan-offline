...
interface Props {
  ...
  onChange: (next: LabelBukuLayout) => void;
  preview: { buku: BukuSample; identity: LibraryIdentity };
}

const FIELD_KINDS: { value: LabelBukuFieldKind; label: string }[] = [
  { value: 'static', label: 'Teks Statis' },
  { value: 'identitas', label: 'Identitas Perpustakaan' },
  { value: 'judul', label: 'Judul Buku' },
  { value: 'kodeBuku', label: 'Kode Buku' },
  { value: 'kodeEksemplar', label: 'Kode Eksemplar' },
  { value: 'pengarang', label: 'Pengarang' },
  { value: 'penerbit', label: 'Penerbit' },
  { value: 'tahun', label: 'Tahun Terbit' },
  { value: 'kodeDdc', label: 'Kode DDC' },
...
export function TemplateEditor({ layout, onChange, preview }: Props) {
  ...
  const updateField = (id: string, patch: Partial<LabelBukuField>) => {
    onChange({
      ...layout,
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-border bg-card p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              variant="outline"
              onClick={addField}
              data-testid="label-buku-field-add"
            >
        ...
        <div className="rounded-lg border border-border bg-card p-4 space-y-2">
          <h3 className="text-sm font-semibold">Dimensi Label (mm)</h3>
...
function FieldEditor({
  ...
  onChange: (patch: Partial<LabelBukuField>) => void;
}) {
  const isText = !['barcode', 'qr', 'rect'].includes(field.kind);
  return (
    <div
      className="rounded-lg border border-border bg-card p-4 space-y-3"
      data-testid="label-buku-field-editor"
    >
      ...
      <div>
        ...
        <Select
          value={field.kind}
          onValueChange={(v) => onChange({ kind: v as LabelBukuFieldKind })}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
      ...
      {field.kind === 'static' && (
        <div>
          ...
          <Input value={field.text ?? ''} onChange={(e) => onChange({ text: e.target.value })} />
        </div>
      )}
      <div className="grid grid-cols-2 gap-2">
      ...
      {field.kind === 'rect' && (
        <div>
          <Label className="text-xs">Warna isi</Label>
          <Input
            type="color"
            value={field.fill ?? '#0f172a'}
