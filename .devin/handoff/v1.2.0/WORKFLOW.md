# v1.2.0 — Workflow Reference

This file is the operational reference for the v1.2.0 batch. The
...
The user has explicitly authorized Devin to squash-merge v1.2.0
...
2. Create release branch `chore/release-v1.2.0`, bump versions, add
   `## [1.1.0]` entry to CHANGELOG.md.
3. Open PR `chore(release): v1.2.0`, wait for CI green, squash-merge.
4. Pull main locally, push tag:
   ```bash
   git fetch origin && git checkout main && git pull
   git tag -a v1.2.0 -m "v1.2.0 — <one-line summary>"
   git push "https://x-access-token:${GITHUB_PAT_ALVIARTS}@github.com/alviarts/perpustakaan-offline.git" v1.2.0
   ```
...
6. Final blocking `message_user` to the user with:
   - Release URL: `https://github.com/alviarts/perpustakaan-offline/releases/tag/v1.2.0`
...
- **Handoff branch** (this batch's manifest):
  `devin/<unix-ts>-v120-handoff`. The first session in the batch
...
3. Open or update a draft PR with body:
   ...
   3. Read .devin/handoff/v1.2.0/BUGS.md section <ITEM-ID>.
   4. Continue from "<one-line context>".
   ```
4. On v120-handoff branch:
